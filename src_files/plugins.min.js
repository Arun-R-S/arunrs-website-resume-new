...
function validateForm(){
    ...
    {
        flag = false;
        $("#f_name").addClass('form-validation-error');
    }
    else{
        $("#f_name").removeClass('form-validation-error');
    }
    if(($("#f_email").val()).length<=0)
    {
        flag = false;
        $("#f_email").addClass('form-validation-error');
    }
    else{
        $("#f_email").removeClass('form-validation-error');
    }
    if(($("#f_organization").val()).length<=0)
    {
        flag = false;
        $("#f_organization").addClass('form-validation-error');
    }
    else{
        $("#f_organization").removeClass('form-validation-error');
    }
    if(($("#f_message").val()).length<=0)
    {
        flag = false;
        $("#f_message").addClass('form-validation-error');
    }
    else{
        $("#f_message").removeClass('form-validation-error');
    }
    if(flag==false)
    {
        return false;
...
$(document).ready(function () {
    ...
    $("#contact-forms").submit(function (e) {
        ...
        $('#contact-forms input').attr('disabled', 'true');
        $('#contact-forms textarea').attr('disabled', 'true');
        var time = new Date();
        var date = time.toLocaleDateString()+' '+time.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true });
        $("#submit-button").css("display", "none");
        $("#submit-inprogress-button").css("display", "block");
        ...
        var sendData = {
            ...
            email: $("#f_email").val(),
            phone: $("#f_phone").val(),
            subject: $('input[name="f_subject"]:checked').val(),
        ...
        function trueStatus(){
            swal("Mail Send", "", "success").then((value) => {
                $('#contact-forms').trigger('reset');
              });
            $("#submit-button").css("display", "block");
            $("#submit-inprogress-button").css("display", "none");
            $('#contact-forms input').removeAttr('disabled', 'true');
            $('#contact-forms textarea').removeAttr('disabled', 'true');
        }

        $.ajax({
            type: 'POSt',
            url: "https://sendprojectmail.herokuapp.com/api/mailService/sendMail",
            headers: {
            ...
            success: function (resultData) {
                if(resultData.sendStatus==true)
                {
                    ...
                    swal("Mail not Send", JSON.stringify(resultData.message), "error");
                    $("#submit-button").css("display", "block");
                    $("#submit-inprogress-button").css("display", "none");
                    $('#contact-forms input').removeAttr('disabled', 'true');
                    $('#contact-forms textarea').removeAttr('disabled', 'true');
            ...
            error: function(jhr, textStatus, errorThrown){
                //return trueStatus();
                swal("Mail not Send", "Backend error", "error");
                    $("#submit-button").css("display", "block");
                    $("#submit-inprogress-button").css("display", "none");
                    $('#contact-forms input').removeAttr('disabled', 'true');
                    $('#contact-forms textarea').removeAttr('disabled', 'true');
